...
def valid(config_path, model_path, timestr):
    ...
    valid_dataset = MFI_Dataset(valid_datasePath, phase=valid_phase, use_dataTransform=valid_use_dataTransform,
                                resize=valid_dataset, imgSzie=valid_imgSize, fusion_type="multi_exposure")
    ...
    with torch.no_grad():
        for valid_step, valid_images in enumerate(valid_dataloader):
            ...
            print(valid_sourceImg1.shape)
            print(valid_sourceImg2.shape)

            if len(valid_images) == 6:
                valid_img1_cr = valid_images[2].to(device)
                valid_img1_cb = valid_images[3].to(device)
                valid_img2_cr = valid_images[4].to(device)
                valid_img2_cb = valid_images[5].to(device)
                # convert YcbCr to RGB
                img_cr = valid_img2_cr
                img_cb = valid_img2_cb

            if len(valid_images) ==4:
                img_cr = valid_images[2].to(device)
                img_cb = valid_images[3].to(device)
            diffusion.sample(model, valid_sourceImg1, valid_sourceImg2, add_noise, concat_type, model_name, model_path,
                             generat_imgs_num, valid_step * valid_batch_size + 1, timestr, valid_step_sum, dataset_name, img_cr, img_cb)


if __name__ == '__main__':
    model_path = "model/epoch_10000.pt"
